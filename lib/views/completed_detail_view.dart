...
class _CompletedDetailViewState extends State<CompletedDetailView> {
  ...
  void _moveToDoCompleted(bool? tapped, ToDo toDo, int index) {
    // TODO this note moved to completed

    setState(() {
      toDo.isCompleted = tapped!;
      items.removeAt(index);
      FileService.createToDo(toDo);
  ...
  void _openTaskDetailPage(ToDo toDo) async{
    ...
    if(result != null && result == "refresh" && mounted) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => DetailScreen(path: toDo.category)));
    }
  }
              ...
              child: Dismissible(
                ...
                child: ListTile(
                  ...
                  leading: Checkbox(
                    ...
                    onChanged: (tapped) => _moveToDoCompleted(tapped, toDo, index),
                  ),
                  title: Text(
                    toDo.taskName,
